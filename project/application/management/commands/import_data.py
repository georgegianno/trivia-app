...
class Command(BaseCommand):
    ...
    help = " \
        ...
        even before the ids are created, and makes the comparisons safe and quick. The uniqueness of relations is ensured \
    ...
    def create_categories_and_questions(self, data):
        ...
        for item in data:
            ...
            check = hash_text(question_text)
            if check in self.check_unique_questions:
                self.duplicate_questions.append(check)
                # self.stdout.write(('Duplicated question: {}, and response: {}'.format(question_text, item)))
                continue
            self.check_unique_questions.append(check)
    ...
    def create_question_answers(self, data):
        ...
        for item in data:
            question_text = html.unescape(item['question'])
            check = hash_text(question_text)
            if check in self.duplicate_questions:
                self.stdout.write(('Duplicated question: {}, and response: {}'.format(question_text, item)))
                continue
            self.check_unique_questions.append(hash_text(item['question']))
            question_hash = hash_text(question_text)
